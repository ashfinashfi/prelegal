"""AI service for legal document chat using LiteLLM with OpenRouter and automatic rate-limit fallbacks."""
...
# Primary model (configurable via OPENROUTER_MODEL env var)
PRIMARY_MODEL = os.getenv("OPENROUTER_MODEL", "openrouter/openai/gpt-oss-20b:free")

# Ordered list of free fallback models in case the primary hits rate limits or upstream 429 errors
FALLBACK_MODELS = [
    PRIMARY_MODEL,
...
# Provider routing: only specify if explicitly set in environment
provider_order = os.getenv("OPENROUTER_PROVIDER")
EXTRA_BODY = {"provider": {"order": [provider_order]}} if provider_order else {}
...
def process_message(messages: list[Message]) -> ChatResponse:
    """Process chat messages with multi-model fallback retry on rate limits (429)."""
    ...
    for model_name in MODELS_TO_TRY:
        try:
            ...
            logger.warning(f"Model {model_name} failed: {e}. Trying next available fallback model...")
            last_exception = e
            continue

    if last_exception:
        raise last_exception
    raise ValueError("All AI models failed to return a response.")
